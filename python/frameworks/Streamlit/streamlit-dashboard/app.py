...
# Extraindo os dados e ordenando por Data
df = pd.read_csv("supermarket_sales.csv", sep=";", decimal=",")
df["Date"] = pd.to_datetime(df["Date"])
df=df.sort_values("Date")

# Montando um filtro por mês
df["Month"] = df["Date"].apply(lambda x: str(x.year) + "-" + str(x.month))
month = st.sidebar.selectbox("Mês", df["Month"].unique())
df_filtered = df[df["Month"] == month]

# layout dividindo a tela em 2 e embaixo 3, colocamos as coisas nessas caixas
col1, col2 = st.columns(2)
col3, col4, col5 = st.columns(3)

# Gráfico 1 na Caixa 1
fig_date = px.bar(df_filtered, x="Date", y="Total", color="City", title="Faturamento por dia")
col1.plotly_chart(fig_date, use_container_width=True)

# Gráfico 2 na Caixa 2
...
# Gráfico 4 na Caixa 4
fig_kind = px.pie(df_filtered, values="Total", names="Payment",
                   title="Faturamento por tipo de pagamento")
col4.plotly_chart(fig_kind, use_container_width=True)


# Gráfico 5 na Caixa 5
