# 🍳 Implementação Prática - Assistente de Receitas com RAG
# Baseado no material da aula sobre LLMs e RAG
...
def analyze_similarity(query_text: str) -> List[Tuple]:
    """
    ...
    Returns:
        List[Tuple]: Lista de tuplas (documento, score) ordenadas por relevância
    """
...
def test_similarity_analysis():
    """Testa a análise de similaridade com diferentes queries."""
    queries_teste = [
...
def create_enhanced_prompt_template() -> str:
    """
...
def get_enhanced_response(query_text: str, context: str) -> str:
    """
...
def add_recipe_metadata() -> Dict:
    ...
    recipe_metadata = {
        "categorias": {
            "cafe_da_manha": ["panqueca", "omelete", "granola", "mingau", "pão", "tapioca"],
            "almoco": ["frango", "peixe", "carne", "salada", "sopa", "arroz"],
            "jantar": ["jantar", "ceia", "light", "sopa"],
            "snacks": ["barrinha", "cookies", "chips", "patê", "sanduíche"],
            "sobremesas": ["bolo", "brownie", "mousse", "brigadeiro", "pudim"]
        },
        "dificuldade": {
            "facil": ["simples", "fácil", "rápido", "básico"],
            "medio": ["médio", "intermediário", "moderado"],
            "dificil": ["difícil", "complexo", "elaborado", "avançado"]
        },
        "tempo_preparo": {
            "rapido": ["rápido", "5 min", "10 min", "15 min"],
            "medio": ["médio", "30 min", "1 hora", "45 min"],
            "lento": ["lento", "2 horas", "demorado", "3 horas"]
        }
    }
    
    return recipe_metadata

def categorize_recipe(recipe_text: str) -> Dict:
    """
...
def filter_recipes_by_category(category: str, query_text: str) -> List[Tuple]:
    """
    ...
    Returns:
        List[Tuple]: Receitas filtradas por categoria
    """
    # Busca geral
...
class RecipeRecommender:
    ...
    def add_to_history(self, user_id: str, recipe_name: str) -> None:
        """
    ...
    def get_user_preferences(self, user_id: str) -> Dict:
        ...
        for entry in history[-10:]:  # Últimas 10 receitas
            ...
            common_ingredients = ["frango", "peixe", "carne", "ovo", "aveia", "banana", 
                                "chocolate", "queijo", "tomate", "cebola", "alho"]
    ...
    def recommend_similar_recipes(self, recipe_name: str, k: int = 3) -> List[Dict]:
        """
        ...
        Returns:
            List[Dict]: Lista de receitas recomendadas
        """
...
def extract_nutritional_info(recipe_text: str) -> Dict:
    """
...
def filter_by_dietary_restrictions(recipes: List, restrictions: List[str]) -> List:
    """
    Filtra receitas por restrições alimentares.
    
    Args:
        recipes (List): Lista de receitas
        restrictions (List[str]): Lista de restrições alimentares
    ...
    restriction_keywords = {
        "vegetariano": ["carne", "frango", "peixe", "bacon", "presunto"],
        "vegano": ["leite", "queijo", "ovo", "manteiga", "mel", "iogurte"],
        "sem_gluten": ["trigo", "farinha", "pão", "macarrão", "cevada"],
        "low_carb": ["açúcar", "farinha", "arroz", "batata", "milho"],
        "sem_lactose": ["leite", "queijo", "iogurte", "creme", "manteiga"]
    }
